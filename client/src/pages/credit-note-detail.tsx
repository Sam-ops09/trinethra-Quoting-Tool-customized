 ...
 * 
 * Displays a credit note with actions to issue, apply, or cancel
 */
...
export default function CreditNoteDetailPage() {
    ...
    const { data: creditNote, isLoading } = useQuery<CreditNote>({
        queryKey: ["/api/credit-notes", params?.id],
        enabled: !!params?.id,
    });

    const issueMutation = useMutation({
        mutationFn: async () => {
            return await apiRequest("POST", `/api/credit-notes/${params?.id}/issue`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/credit-notes", params?.id] });
    ...
    const applyMutation = useMutation({
        mutationFn: async () => {
            return await apiRequest("POST", `/api/credit-notes/${params?.id}/apply`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/credit-notes", params?.id] });
            queryClient.invalidateQueries({ queryKey: ["/api/credit-notes"] });
            ...
            toast({
                ...
                description: "The credit has been applied to the invoice successfully.",
            });
        },
        onError: () => {
            toast({
    ...
    const cancelMutation = useMutation({
        mutationFn: async () => {
            return await apiRequest("POST", `/api/credit-notes/${params?.id}/cancel`);
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/credit-notes", params?.id] });
            queryClient.invalidateQueries({ queryKey: ["/api/credit-notes"] });
    ...
    const selectedStatusIndex = STATUS_STEPS.findIndex((s) => s.id === creditNote?.status);
                    ...
                    <Card className="text-center">
                        ...
                        <CardContent>
                            <p className="text-sm sm:text-base text-muted-foreground">
                                The requested credit note could not be located.
                            </p>
                            <Button
                                className="mt-4"
    ...
    const canIssue = creditNote.status === "draft" && isFeatureEnabled("creditNotes_issue");
    const canApply = creditNote.status === "issued" && isFeatureEnabled("creditNotes_apply") && !!creditNote.invoiceId;
                        ...
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    <Badge variant="outline" className={`${getStatusColor(creditNote.status)} text-xs px-2 py-0.5 h-6`}>
                                        {creditNote.status}
                                    </Badge>
                                </div>
                            </div>
                            ...
                            return (
                                <div key={step.id} className="flex items-center gap-2 shrink-0">
                                    ...
                                    {index < STATUS_STEPS.length - 1 && (
                                        <div className={`h-px w-6 sm:w-12 transition-colors ${isCompleted ? "bg-emerald-600" : "bg-slate-200 dark:bg-slate-800"}`} />
                                    )}
                                </div>
                            );
                        })}
            ...
            <div className="flex-1 w-full max-w-[1800px] mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6">
                <div className="grid gap-4 sm:gap-6 lg:grid-cols-[1fr_340px] xl:grid-cols-[1fr_380px]">
                                    ...
                                    {creditNote.invoiceId ? (
                                        <>
                                            <Button
                                                ...
                                                onClick={() => setLocation(`/invoices/${creditNote.invoiceId}`)}
                                            >
                                                {creditNote.invoice?.invoiceNumber || "View Invoice"}
                                            </Button>
                                            {creditNote.invoice && (
                                                <p className="text-xs text-slate-500 mt-2 text-center">
                                                    Invoice Total: {formatCurrency(creditNote.invoice.total)}
                                                </p>
                                            )}
                                        </>
                                    ) : (
                                        <p className="text-sm text-slate-500 text-center italic py-2">
                                            Standalone Credit Note (No Invoice Linked)
                                        </p>
                                    )}
                                </CardContent>
                            </Card>
                        ...
                        <Card className="border-slate-200 dark:border-slate-800">
                            <CardHeader className="p-3 sm:p-4 pb-2 border-b border-slate-200 dark:border-slate-800">
                                <CardTitle className="text-xs sm:text-sm font-bold">
                                    Line Items ({creditNote.items?.length || 0})
                                </CardTitle>
                            </CardHeader>
                                ...
                                {creditNote.items?.map((item) => (
                                    ...
                                    >
                                        ...
                                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs text-slate-600 dark:text-slate-400">
                                            ...
                                            <div>Total: <span className="font-medium text-emerald-600">{formatCurrency(String(item.quantity * parseFloat(item.unitPrice)))}</span></div>
                                        </div>
                                    </div>
                                ))}
                            </CardContent>
                    ...
                    <div className="space-y-4 sm:space-y-6 lg:sticky lg:top-24 self-start">
                        ...
                        <Card className="border-slate-200 dark:border-slate-800">
                            <CardHeader className="p-3 sm:p-4 pb-2 border-b border-slate-200 dark:border-slate-800">
                                <CardTitle className="text-xs sm:text-sm font-bold">Credit Summary</CardTitle>
                            </CardHeader>
                            <CardContent className="p-3 sm:p-4 space-y-2 text-xs sm:text-sm">
                                <div className="border-b border-slate-200 dark:border-slate-800 pb-2 flex justify-between items-center">
                                    <span className="font-bold text-slate-900 dark:text-white">Total Credit</span>
                        ...
                        <Card className="border-slate-200 dark:border-slate-800">
                            ...
                            <CardContent className="p-3 sm:p-4 space-y-2 text-xs sm:text-sm">
                                <div className="flex justify-between">
                                    ...
                                    <span className="font-medium text-slate-900 dark:text-white">{formatDate(creditNote.createdAt)}</span>
                                </div>
                                <div className="flex justify-between">
                                    <span className="text-slate-600 dark:text-slate-400">Issued</span>
                                    <span className="font-medium text-slate-900 dark:text-white">{formatDate(creditNote.issueDate)}</span>
                                </div>
                                {creditNote.appliedDate && (
                                    <div className="flex justify-between">
                                        <span className="text-slate-600 dark:text-slate-400">Applied</span>
                                        <span className="font-medium text-emerald-600">{formatDate(creditNote.appliedDate)}</span>
                                    </div>
                                )}
                            </CardContent>
                        </Card>
            ...
            <AlertDialog open={showApplyDialog} onOpenChange={setShowApplyDialog}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            This will apply a credit of <strong>{formatCurrency(creditNote.total)}</strong> to invoice{" "}
                            <strong>{creditNote.invoice?.invoiceNumber}</strong>. This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            ...
                            <strong>{creditNote.creditNoteNumber}</strong>? This action cannot be undone.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
