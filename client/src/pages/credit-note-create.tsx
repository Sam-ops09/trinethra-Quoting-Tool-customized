/**
 * Credit Note Create/Edit Page
 * 
 * Form for creating or editing a credit note linked to an invoice
 */
...
import { Plus, Trash2, Loader2, ArrowLeft, Home, ChevronRight, CreditCard, Edit } from "lucide-react";
...
export default function CreditNoteCreate() {
    ...
    // Fetch existing credit note if editing
    const { data: existingNote, isLoading: isLoadingNote } = useQuery({
        queryKey: ["/api/credit-notes", id],
        queryFn: async () => {
            const res = await fetch(`/api/credit-notes/${id}`, { credentials: "include" });
    ...
    const createMutation = useMutation({
        mutationFn: async (values: z.infer<typeof creditNoteFormSchema>) => {
            const url = isEdit ? `/api/credit-notes/${id}` : "/api/credit-notes";
        ...
        onSuccess: (data) => {
            ...
            toast({
                title: "Success",
                description: isEdit ? "Credit note updated successfully" : "Credit note created successfully",
            });
            setLocation(`/credit-notes/${data.id}`);
        },
        onError: (error: any) => {
            toast({
                title: "Error",
    ...
    const onSubmit = (values: z.infer<typeof creditNoteFormSchema>) => {
                                            ...
                                            <FormField
                                                ...
                                                render={({ field }) => (
                                                    <FormItem>
                                                        ...
                                                        <FormControl>
                                                            <Input {...field} placeholder="e.g., Product return, Damaged goods" />
                                                        </FormControl>
                                                        <FormMessage />
                                                    </FormItem>
                                                )}
