 ...
 * 
 * Displays all credit notes with filtering and navigation
 */
...
export default function CreditNotes() {
    ...
    const filtered = useMemo(() => creditNotes?.filter((cn) => {
        ...
        const matchStatus = status === "all" || cn.status === status;
        return matchSearch && matchStatus;
    }), [creditNotes, search, status]);

    const stats = useMemo(() => ({
        total: creditNotes?.length || 0,
        draft: creditNotes?.filter((c) => c.status === "draft").length || 0,
        issued: creditNotes?.filter((c) => c.status === "issued").length || 0,
        applied: creditNotes?.filter((c) => c.status === "applied").length || 0,
        cancelled: creditNotes?.filter((c) => c.status === "cancelled").length || 0,
    }), [creditNotes]);
    ...
    if (!isFeatureEnabled("creditNotes_module")) {
        return (
            <div className="container mx-auto py-8">
                <Card>
                    <CardContent className="p-8 text-center">
                        <p className="text-muted-foreground">Credit Notes feature is disabled.</p>
                    </CardContent>
                </Card>
            </div>
        );
                ...
                <div className="w-full max-w-[1800px] mx-auto px-3 sm:px-4 lg:px-6 py-2.5">
                    <div className="flex items-center justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            {isFeatureEnabled("creditNotes_create") && (
                                <PermissionGuard resource="invoices" action="create">
                                    <Button
                                        ...
                                        onClick={() => setLocation("/credit-notes/new")}
                                    >
                    ...
                    <div className="mt-2.5 flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">
                        {[
                            { key: "total", label: "Total", value: stats.total, icon: CreditCard, color: "text-slate-600 dark:text-slate-400" },
                            { key: "draft", label: "Draft", value: stats.draft, icon: Edit, color: "text-slate-600 dark:text-slate-400" },
                            { key: "issued", label: "Issued", value: stats.issued, icon: Send, color: "text-blue-600 dark:text-blue-400" },
                            { key: "applied", label: "Applied", value: stats.applied, icon: CheckCircle2, color: "text-emerald-600 dark:text-emerald-400" },
                            ...
                            <div className="grid gap-3 sm:gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
                                {filtered.map((cn) => {
                                    ...
                                    return (
                                        <Card
                                            key={cn.id}
                                            onMouseEnter={() => setHoveredCard(cn.id)}
                                            onMouseLeave={() => setHoveredCard(null)}
                                            onClick={() => setLocation(`/credit-notes/${cn.id}`)}
                                            className={cn.constructor === String ? "" : `group relative border-slate-200 dark:border-slate-800 cursor-pointer transition-all duration-300 overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-slate-950/50 hover:-translate-y-1 ${hoveredCard === cn.id ? "ring-2 ring-emerald-500/50" : ""}`}
                                        >
                                            {/* Progress Bar */}
                                                ...
                                                <div className="flex items-start justify-between gap-3">
                                                    <div className="flex-1 min-w-0">
                                                        ...
                                                        <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400">
                                                            ...
                                                            {formatDate(cn.issueDate || cn.createdAt)}
                                                        </div>
                                                    </div>
                                                    <Badge
                                                        variant="outline"
                                            ...
                                            <CardContent className="p-4 pt-0 space-y-3">
                                                ...
                                                <div className="flex items-center gap-3 p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <p className="text-sm font-bold text-slate-900 dark:text-white truncate">
                                                            {cn.client?.name || "Unknown Client"}
                                                        </p>
                                                        <p className="text-xs text-slate-600 dark:text-slate-400 truncate">
                                                            Invoice: {cn.invoice?.invoiceNumber || "-"}
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex items-end justify-between">
                                                    ...
                                                    <div className={`flex items-center gap-1 transition-all duration-300 ${hoveredCard === cn.id ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"}`}>
                                                        <Button
                                                            variant="ghost"
                                                            size="icon"
                                                            ...
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                setLocation(`/credit-notes/${cn.id}`);
                                                            }}
                                                        >
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <p className="text-sm text-slate-600 dark:text-slate-400">
                                                {search
                                                    ...
                                                    : "Create your first credit note for refunds or returns."}
                                            </p>
                                        </div>
                                        {!search && isFeatureEnabled("creditNotes_create") && (
                                            <Button
                                                size="lg"
