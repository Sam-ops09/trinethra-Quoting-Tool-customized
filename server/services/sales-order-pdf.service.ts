...
export class SalesOrderPDFService {
    ...
    static generateSalesOrderPDF(data: SalesOrderPdfData): PDFKit.PDFDocument {
        ...
        // Items logic specific to SO (reuse similar table structure)
    ...
    private static drawHeader(
        ...
        try {
            ...
            if (fs.existsSync(logoPath)) {
                doc.image(logoPath, x, topY, { fit: [50, 50] });
            }
        } catch (e) {
        ...
        doc.text(data.companyName, x, topY + 60, { width: w });

        // Company Address
        doc.font("Helvetica").fontSize(9).fillColor(this.SUBTLE);
        doc.text(this.normalizeAddress(data.companyAddress, 3), x, topY + 78, { width: w / 2 });
        ...
        if (contactParts.length > 0) {
            doc.text(contactParts.join(" | "), x, doc.y + 5, { width: w / 2 });
        }

        // Title: SALES ORDER
        doc.font("Helvetica-Bold").fontSize(16).fillColor(this.INK);
        doc.text("SALES ORDER", this.MARGIN_LEFT, 50, {
        ...
        // Box background
        this.box(doc, rightColX, infoY, rightColW, 60, { fill: "#F8FAFC", stroke: this.LINE });
        ...
        doc.y = Math.max(doc.y, topY + 120); // Ensure enough space after header
        this.hr(doc);
        doc.y += 15;
    }
    ...
    private static drawTopBlocks(
        ...
        // ========== Company Details Section (above Ship To) ==========
        ...
        if (hasCompanyDetails) {
            ...
            if (data.companyEmail) { // Using data.companyEmail for contact in SO context
                companyInfo.push(`Contact: ${String(data.companyEmail).trim()}`);
            }
        ...
        // ========== Original Ship To / Bill To boxes ==========
        ...
        const row = (i: number, label: string, value: string) => {
            ...
            // Don't draw separator line after the last row (row 5)
            if (i < 5) {
                doc.save();
        ...
        const quoteNo = String((data.quote as any)?.quoteNumber || "-"); // Assuming quoteNumber might still be relevant for SO
        const preparedBy = String(data.companyDetails.name || "-"); // Using company name as prepared by for SO
        ...
        // Delivery notes: clean and pass to row (row function handles empty strings)
    ...
    // ---------------------------
    private static drawItemsTable(doc: InstanceType<typeof PDFDocument>, data: SalesOrderPdfData) {
        doc.y += 20;
        ...
        this.box(doc, this.MARGIN_LEFT, y, this.CONTENT_WIDTH, h, { fill: "#F1F5F9", stroke: this.LINE });
        ...
        for (let idx = 0; idx < items.length; idx++) {
            ...
            // FIX: if appendix exists, keep inline serial line SHORT so it never cuts “see appendix”
            ...
            const descLinesAll = this.wrapTextLines(doc, desc, col.desc - 12, 3);
            ...
            if (serialInline) {
                ...
                // allow up to 2 lines, but with the shortened string it typically stays 1
    ...
    private static drawFinalSections(doc: InstanceType<typeof PDFDocument>, data: SalesOrderPdfData) {
        ...
        // ---- Totals height (reserves space for tax line; prevents overlap) ----
        const rowH = 16;
        ...
        // If terms were longer than shown, continue on new page
    ...
    private static drawDeclarationBankAndSignatures(
        ...
        // Two columns: left declaration, right bank
        const gap = 10;
        const leftW = w * 0.56;
        const rightW = w - leftW - gap;
        ...
        // Computer generated note (inside margins; does not force new pages)
        const oldY = doc.y;
    ...
    private static parseSerialNumbers(raw: any): string[] {
        ...
        try {
            ...
            if (typeof raw === "string") {
                ...
                // allow comma/newline/semicolon separated
                return t
                    .split(/,|\n|;/g)
                    .map((s) => s.trim())
                    .filter(Boolean);
    ...
    private static serialInlineSummary(serials: string[], hasAppendix: boolean): string {
        ...
        // FIX: If appendix exists, keep this SHORT so it never truncates “see appendix”
        if (hasAppendix) {
        ...
        // No appendix: show a richer inline preview
